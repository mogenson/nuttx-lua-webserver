...
-- lua 5.1 compat
local pack = (table.unpack or require("atlas.utils").pack)
...
function Route.run(self, request)
    local raw_parameters = pack(string.match(request.path, self.path_pattern))
    ...
    return self.controller(request, unpack(parameters))
end
